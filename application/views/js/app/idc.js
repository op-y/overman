var dts = $('#idcs_table').dataTable({
    ...
    "columnDefs": [{
         ...
         "render" : function(data, type, row) {
             ...
             var idcCode = data.code;
             var idcAddress = data.address;
             var idcAdministrator = data.administrator;
             var idcTel = data.tel;
             var html = "<a href='javascript:void(0);' onclick='showUpdate("+idcId+",\""+idcName+"\",\""+idcCode+"\",\""+idcAddress+"\",\""+idcAdministrator+"\",\""+idcTel+"\")' class='btn btn-warning btn-xs btn-del'>修改</a>";
             html += "&nbsp;";
...
function showUpdate(id, name, code, address, administrator, tel){
    fill(id, name, code, address, administrator, tel);
    $("#idc_modal").modal("toggle");
}
...
function add() {
    ...
    var valueCode = $("#idc_modalField_code").val();
    ...
    $.ajax({ 
        ...
        data:{name:valueName, code:valueCode, address:valueAddress, administrator:valueAdministrator, tel:valueTel},
...
function update() {
    ...
    $.ajax({ 
        type:"POST",
        url:"./ajaxUpdateIdc", 
        data:{id:valueId, name:valueName, code:valueCode, address:valueAddress, administrator:valueAdministrator, tel:valueTel},
        datatype: "json",
        beforeSend:function(){
        },
        complete: function(){
